...
public class DelaunayTest : MonoBehaviour
{
    public int NumberOfVertices = 100;
        ...
        [SerializeField] private GetPointPos _getPointPos;
        ...
        public void CalculateDelaunay(Text textArea)
        {
            lineMaterial = new Material(Shader.Find("Hidden/Internal-Colored"));
            var list = ARMingle();
            Vertex3[] vertices = new Vertex3[list.Count];
            textArea.text += "ARMingle count:" + list.Count;
            textArea.text += "Vertices count:" + vertices.Length;
        ...
        private List<Vector3> ARMingle()
        {
            var list = new List<Vector3>();
            foreach (var point in _getPointPos.Points)
            {
                NumberOfVertices++;
